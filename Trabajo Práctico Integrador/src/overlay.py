...
def dibujar_punto_de_tracking(frame, punto, track_id, color=COLOR_TUPLA_BLANCO):
    ...
    cv2.circle(frame, (x, y), RADIO_CIRCULO_EXTERIOR_ID_TRACKING, color, GROSOR_CONTORNO_TEXTO_DOS_PIXELES)
...
def dibujar_zona(frame, poly, color=COLOR_TUPLA_ROJO, thickness=GROSOR_CONTORNO_TEXTO_DOS_PIXELES, alpha=TRANSPARENCIA_ZONA, zone_name=None, zone_id=None):
    ...
    if zone_name or zone_id is not None:
        ...
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, ESCALA_FUENTE_CINCUENTA_PORCIENTO, GROSOR_CONTORNO_TEXTO_DOS_PIXELES)
        ...
        cv2.putText(frame, label, (x_label, y_label), cv2.FONT_HERSHEY_SIMPLEX, ESCALA_FUENTE_CINCUENTA_PORCIENTO, color, GROSOR_CONTORNO_TEXTO_DOS_PIXELES, cv2.LINE_AA)
