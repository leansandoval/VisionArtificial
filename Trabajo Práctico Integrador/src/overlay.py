...
ESCALA_FUENTE_BOUNDING_BOX = 0.6
ESCALA_FUENTE_ID_TRACKING = 0.5
...
def dibujar_bounding_box(frame, bounding_box, label=None, color=COLOR_TUPLA_VERDE, grosor=GROSOR_BOUNDING_BOX):
    ...
    if label:
        ...
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, ESCALA_FUENTE_BOUNDING_BOX, GROSOR_CONTORNO_TEXTO_DOS_PIXELES)
        ...
        cv2.putText(frame, label, punto_texto, cv2.FONT_HERSHEY_SIMPLEX, ESCALA_FUENTE_BOUNDING_BOX, COLOR_TUPLA_NEGRO, GROSOR_CONTORNO_TEXTO_DOS_PIXELES, cv2.LINE_AA)
...
def dibujar_punto_de_tracking(frame, punto, track_id, color=COLOR_TUPLA_BLANCO):
    ...
    (tw, th), _ = cv2.getTextSize(str(track_id), cv2.FONT_HERSHEY_SIMPLEX, ESCALA_FUENTE_ID_TRACKING, GROSOR_CONTORNO_TEXTO_UN_PIXEL)
    punto_texto = (x - tw // 2, y + 20)
    cv2.putText(frame, str(track_id), punto_texto, cv2.FONT_HERSHEY_SIMPLEX, ESCALA_FUENTE_ID_TRACKING, color, GROSOR_CONTORNO_TEXTO_DOS_PIXELES, cv2.LINE_AA)

# Dibuja zona poligonal con nombre opcional
def draw_zone(frame, poly, color=(0,0,255), thickness=GROSOR_CONTORNO_TEXTO_DOS_PIXELES, alpha=0.08, zone_name=None, zone_id=None):
    ...
    if zone_name or zone_id is not None:
        ...
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, GROSOR_CONTORNO_TEXTO_DOS_PIXELES)
        ...
        cv2.putText(
            ...
            0.7,
            color,
            GROSOR_CONTORNO_TEXTO_DOS_PIXELES,
            cv2.LINE_AA
        )
...
def draw_fps_professional(frame, fps, frame_number=None):
    ...
    (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, GROSOR_CONTORNO_TEXTO_DOS_PIXELES)
    # Fondo semi-transparente
    overlay = frame.copy()
    cv2.rectangle(overlay, (8, 8), (tw + 22, th + 22), COLOR_TUPLA_NEGRO, GROSOR_RELLENO_COMPLETO)
    cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
    # Texto
    cv2.putText(frame, text, (15, th + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, COLOR_TUPLA_VERDE, 2, cv2.LINE_AA)
...
def draw_stats_panel(frame, stats_dict, position='top-right'):
    ...
    for key, value in stats_dict.items():
        ...
        cv2.putText(
            ...
            0.5,
            COLOR_TUPLA_BLANCO,
            GROSOR_CONTORNO_TEXTO_UN_PIXEL,
            cv2.LINE_AA
        )
        y_offset += 25
